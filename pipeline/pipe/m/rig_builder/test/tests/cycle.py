...
class TestLargeCyclesDG(RigBuildTest):
    ...
    def run(self):
        ...
        large_cycle_clusters = _get_cycles_from_nodes(
            dg_cycle_nodes, threshold=self.CYCLE_THRESHOLD
        )
        cycle_sizes_and_names = (
            (len(cluster), cluster[0]) for cluster in large_cycle_clusters
        )
...
class TestCyclesDG(RigBuildTest):
    ...
    def run(self):
        # Dependency Graph Cycle
        has_cycles: bool = cmds.cycleCheck(all=True)  # type: ignore

        if has_cycles:
            dg_cycle_nodes: list[str] = cmds.cycleCheck(all=True, list=True) or []  # type: ignore

            large_cycle_clusters = _get_cycles_from_nodes(dg_cycle_nodes)
            cycle_sizes_and_names = (
                (len(cluster), cluster[0]) for cluster in large_cycle_clusters
            )
            ...
            formatted_clusters = "\n".join(cluster_log_strings)
            self.log_warn(f"Scene has DG cycle(s): {formatted_clusters}")
            return False
        else:
            self.log_success()
            return True
